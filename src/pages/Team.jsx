  ...
  return (
    <div className="min-h-screen bg-black text-white pt-24 pb-20 overflow-hidden relative">
      ...
      <div className="container mx-auto px-6 lg:px-12 relative z-10" ref={containerRef}>
        ...
        {/* 5-Column Grid (Desktop) / 2-Column Grid (Mobile) */}
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 md:gap-6">
          {teamData.map((member, index) => (
            <div 
              key={member.id}
              ref={el => cardsRef.current[index] = el}
              className="group relative bg-[#0a0a0a] border border-white/10 overflow-hidden hover:border-red-600/50 transition-all duration-300"
            >
              ...
              <div className="relative aspect-[3/4] overflow-hidden bg-black/50">
                 ...
                 <div className="absolute bottom-0 left-0 w-full flex">
                    {[...Array(5)].map((_, i) => (
                      <div 
                        key={i} 
                        className={`flex-1 h-1 ${i < member.clearanceLevel ? 'bg-jurassic-yellow' : 'bg-white/10'} border-r border-black/50`}
                      ></div>
                    ))}
                 </div>
              ...
              <div className="p-5 relative z-10">
                 ...
                 <div className="space-y-2 border-t border-white/10 pt-4">
                   <div className="flex justify-between items-center text-[9px] font-black tracking-widest uppercase">
                     ...
                     <span className="text-white/60 text-right">{member.department}</span>
                   </div>
                   <div className="flex justify-between items-center text-[9px] font-black tracking-widest uppercase">
                     <span className="text-white/30">ID:</span>
                     <span className="text-white/60">#SYS-{member.id.toString().padStart(3, '0')}</span>
                   </div>
                 </div>
